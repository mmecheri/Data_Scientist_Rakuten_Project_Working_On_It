...
from src.data_preprocessing.data_loader import get_relative_path  # si tu as ça sinon tu vires
...
def generate_submission_file(mapped_preds_df, sub_data, output_name, sub_dir):
    """
    Generate the final submission file in the required format:
    - Index must match sub_data index.
    - Only 'prdtypecode' column must appear.

    Parameters
    ----------
    mapped_preds_df : pd.DataFrame
        DataFrame containing mapped predictions with columns: ['Predicted Encoded', 'prdtypecode', 'Label']

    sub_data : pd.DataFrame
        Original submission dataset with correct index.

    output_name : str
        Name of the output submission file (e.g., 'submission_v1.csv')

    submission_dir : Path
        Directory where the submission file will be saved.
    """

    print(f"[INFO] Generating submission file: {output_name}")

    # Align index with sub_data
    mapped_preds_df.index = sub_data.index

    submission_df = pd.DataFrame({
        'prdtypecode': mapped_preds_df['prdtypecode']
    }, index=sub_data.index)

    # Create directory if not exists
    submission_dir = Path(sub_dir)
    submission_dir.mkdir(parents=True, exist_ok=True)

    # Define final output path
    output_path = submission_dir / output_name

    submission_df.to_csv(output_path, header=True)

    print(f"[✔] Submission file saved at: {get_relative_path(output_path) if 'get_relative_path' in globals() else output_path}")
